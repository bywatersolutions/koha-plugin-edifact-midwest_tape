...
say "\nNot koha-plugin-edifact-midwest-tape, exiting" && exit 0 unless $repo eq "koha-plugin-edifact-midwest-tape";
...
foreach my $repo (@repos) {
    next if $repo eq 'koha-plugin-edifact-midwest-tape';
    next if $repo eq 'koha-plugin-edifact-midwest-tape-docs';
